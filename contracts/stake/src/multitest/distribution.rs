...
use cosmwasm_std::{assert_approx_eq, Addr, Decimal256, Uint128, Uint256};
...
fn multiple_distribution_flows() {
    let members = vec![
        MockApi::default().addr_make("member1"),
        MockApi::default().addr_make("member2"),
        MockApi::default().addr_make("member3"),
        MockApi::default().addr_make("member4"),
    ];
    ...
    assert_eq!(
        suite
            .query_balance(&suite.stake_contract(), "juno")
            .unwrap(),
        400,
    );
    assert_eq!(
        suite
            .query_balance(&suite.stake_contract(), "luna")
            .unwrap(),
        400,
    );
...
fn mass_bond_with_multiple_distribution_flows() {
    ...
    let richie = MockApi::default().addr_make("richie rich");
    let bonds = vec![5_000u128, 10_000u128, 25_000u128];
    let delegated: u128 = bonds.iter().sum();
    let unbonding_period = 1000u64;
...
#[test]
fn calculate_apr() {
    let distributor = MockApi::default().addr_make("distributor");
    let members = vec![
        MockApi::default().addr_make("member1"),
        MockApi::default().addr_make("member2"),
        MockApi::default().addr_make("member3"),
    ...
    let member1 = members[0].clone();
    let member2 = members[1].clone();
    ...
    suite
        .create_distribution_flow(
            ...
            AssetInfo::Native("juno".to_string()),
            vec![
                (unbonding_periods[0], Decimal256::percent(50)),
                (unbonding_periods[1], Decimal256::one()),
                (unbonding_periods[2], Decimal256::percent(300)),
            ],
        )
        .unwrap();
    ...
    let annual_rewards = suite.query_annualized_rewards().unwrap();
    assert_eq!(annual_rewards[0].1[0].amount, Some(Decimal256::zero()));
    assert_eq!(annual_rewards[1].1[0].amount, Some(Decimal256::zero()));
    assert_eq!(annual_rewards[2].1[0].amount, Some(Decimal256::zero()));
    ...
    assert_eq!(
        ...
        Uint256::from(1000u128).mul_floor(annual_rewards[0].1[0].amount.unwrap()),
        Uint256::from(2607u128),
    );
    assert_eq!(
        Uint256::from(1000u128).mul_floor(annual_rewards[1].1[0].amount.unwrap()),
        Uint256::from(5214u128),
    );
    assert_eq!(
        Uint256::from(1000u128).mul_floor(annual_rewards[2].1[0].amount.unwrap()),
        Uint256::from(15642u128),
    );
...
fn apr_cw20() {
    ...
    // create a cw20 token
    let owner = MockApi::default().addr_make("admin");
    ...
    suite
        .create_distribution_flow(
            ...
            vec![
                (unbonding_periods[0], Decimal256::percent(70)),
                (unbonding_periods[1], Decimal256::one()),
                (unbonding_periods[2], Decimal256::percent(200)),
            ],
        )
        .unwrap();
    ...
    assert_eq!(
        // multiply by 1000 to get an int of promille. eg 123.4 % = 1.234 * 1000 = 1234
        Uint256::from(1000u128).mul_floor(annual_rewards[0].1[0].amount.unwrap()),
        Uint256::from(1489361702u128),
    );
    assert_eq!(
        Uint256::from(1000u128).mul_floor(annual_rewards[1].1[0].amount.unwrap()),
        Uint256::from(2127659574u128),
    );
    assert_eq!(
        Uint256::from(1000u128).mul_floor(annual_rewards[2].1[0].amount.unwrap()),
        Uint256::from(4255319148u128),
    );
...
fn simple_apr_simulation() {
    ...
    assert_eq!(
        ...
        Uint256::from(stakes[0]).mul_floor(expected_reward_per_token[0]),
    );
    let actual_reward = suite
    ...
    assert_eq!(
        actual_reward.amount,
        Uint256::from(stakes[1]).mul_floor(expected_reward_per_token[1]),
    );
}
...
fn divisible_amount_distributed_twice() {
    ...
    suite
        .create_distribution_flow(
            ...
            &members[0],
            AssetInfo::Native("juno".to_string()),
            vec![(unbonding_period, Decimal256::percent(200))],
        )
        .unwrap();
...
fn divisible_amount_distributed_twice_accumulated() {
    ...
    assert_eq!(
        suite
            .query_balance_vesting_contract(&Addr::unchecked(suite.token_contract()))
            .unwrap(),
        0
    );
    assert_eq!(suite.query_balance(&members[0], "juno").unwrap(), 125);
...
fn points_changed_after_distribution() {
    let members = vec![
        MockApi::default().addr_make("member0"),
        MockApi::default().addr_make("member1"),
        MockApi::default().addr_make("member2"),
        MockApi::default().addr_make("member3"),
    ];
...
fn cannot_withdraw_others_funds() {
    ...
    let err = suite
        .withdraw_funds(&members[0], members[1].as_ref(), None)
        .unwrap_err();

    assert!(err.to_string().contains(&ContractError::Unauthorized {}.to_string()));

    suite
        .withdraw_funds(&members[1], members[1].as_ref(), None)
        .unwrap();
...
fn funds_withdrawal_delegation() {
    ...
    assert_eq!(suite.delegated(&members[0]).unwrap(), members[0]);
    assert_eq!(suite.delegated(&members[1]).unwrap(), members[1]);
    ...
    suite
        .delegate_withdrawal(&members[1], members[0].as_str())
        .unwrap();
    ...
    assert_eq!(suite.delegated(&members[0]).unwrap(), members[0]);
    assert_eq!(suite.delegated(&members[1]).unwrap(), members[0]);
...
fn querying_unknown_address() {
    ...
    let resp = suite
        .withdrawable_rewards(&MockApi::default().addr_make("unknown"))
        .unwrap();
    assert_eq!(resp, vec![]);
}
...
fn rebond_works() {
    ...
    let executor = MockApi::default().addr_make("executor");
    ...
    suite
        .create_distribution_flow(
            ...
            AssetInfo::Native("juno".to_string()),
            vec![
                (unbonding_period, Decimal256::one()),
                (unbonding_period2, Decimal256::percent(200)),
            ],
        )
        .unwrap();
...
fn rebond_multiple_works() {
    ...
    suite
        .create_distribution_flow(
            ...
            vec![
                (unbonding_period, Decimal256::one()),
                (unbonding_period2, Decimal256::one()),
            ],
        )
        .unwrap();
...
fn multiple_rewards() {
    ...
    // distribute juno
    suite
        .distribute_funds(&executor, Some(&executor), Some(juno(1_000)))
        .unwrap();
    // distribute wynd
    suite
        .distribute_funds(
...
fn distribute_staking_token_should_fail() {
    ...
    assert!(err.to_string().contains(&ContractError::InvalidAsset {}.to_string()));
}

#[test]
fn unbond_after_new_distribution() {
    let members = [
        MockApi::default().addr_make("member"),
        MockApi::default().addr_make("executor"),
    ];
    let member = members[0].clone();
    let executor = members[1].clone();
    ...
    suite
        .create_distribution_flow(
            ...
            &executor,
            AssetInfo::Native("juno".to_string()),
            vec![(100, Decimal256::one())],
        )
        .unwrap();

    // unbond
    suite
        .unbond(&MockApi::default().addr_make("member"), 1_000, 100)
        .unwrap();
}
...
fn distribution_respects_min_bond() {
    ...
    // distribute
    suite
        .distribute_funds(&executor, Some(&executor), Some(juno(300)))
        .unwrap();
...
fn withdraw_adjustment_handled_lazily() {
    ...
    let executor = MockApi::default().addr_make("executor");
    let member = MockApi::default().addr_make("member");
    let mut suite = SuiteBuilder::new()
        .with_admin(&MockApi::default().addr_make("admin"))
        .with_unbonding_periods(vec![100])
        .with_min_bond(0)
    ...
    // withdraw
    suite
        .withdraw_funds(&member, member.to_string().as_str(), None)
        .unwrap();
