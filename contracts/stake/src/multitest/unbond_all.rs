...
fn execute_unbond_all_case() {
    ...
    let err = suite
        .execute_unbond_all(&MockApi::default().addr_make("fake_unbonder"))
        .unwrap_err();
    ...
    suite
        .execute_unbond_all(&MockApi::default().addr_make(UNBONDER))
        .unwrap();
...
fn execute_stop_unbond_all_case() {
    ...
    let mut err = suite
        .execute_stop_unbond_all(&MockApi::default().addr_make(UNBONDER))
        .unwrap_err();

    assert!(err
        .to_string()
        .contains(&ContractError::FlagAlreadySet {}.to_string()));

    err = suite
        .execute_stop_unbond_all(&MockApi::default().addr_make(ADMIN))
        .unwrap_err();
    ...
    err = suite
        .execute_stop_unbond_all(&MockApi::default().addr_make("user"))
        .unwrap_err();
    assert!(err
        .to_string()
        .contains(&ContractError::Unauthorized {}.to_string()));
...
fn delegate_with_unbond_all_flag() {
    ...
    assert!(err
        .to_string()
        .contains(&ContractError::CannotDelegateIfUnbondAll {}.to_string()));
}

#[test]
fn delegate_as_with_unbond_all_flag() {
    let user = MockApi::default().addr_make("factory2");
    let user2 = MockApi::default().addr_make("factory3");
    let mut suite = SuiteBuilder::new()
        .with_initial_balances(vec![(&user, 100_000)])
        .with_unbonder(&MockApi::default().addr_make(UNBONDER))
        .build();
...
fn rebond_with_unbond_all_flag() {
    ...
    assert!(err
        .to_string()
        .contains(&ContractError::CannotRebondIfUnbondAll {}.to_string()));
}
